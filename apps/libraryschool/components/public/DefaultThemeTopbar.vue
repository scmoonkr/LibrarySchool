    ...
    >
      ...
      <nav v-if="!hideNav" :class="['theme-nav', { open: isMobileMenuOpen }]">
        ...
        <template v-if="user">
          ...
          <button class="theme-mobile-menu-action" type="button" @click="logout">로그아웃</button>
        </template>
      </nav>

      <div v-if="!hideNav" class="theme-auth">
        <NuxtLink v-if="!user" class="theme-login" to="/login">로그인</NuxtLink>
        <div v-else ref="accountMenuRef" class="theme-user">
          ...
          <div v-if="isAccountMenuOpen" class="theme-account-menu">
            <NuxtLink to="/account/profile" @click="closeAccountMenu">회원정보 수정</NuxtLink>
            <NuxtLink to="/account" @click="closeAccountMenu">마이 페이지</NuxtLink>
